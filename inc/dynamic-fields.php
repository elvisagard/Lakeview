...
//PPX-DYNFLDS 1.3 POPULATE ORDERBY FIELD
function ppx_orderby_fields( $field ) {

        $field['choices'] = array();
        $field['choices']['date'] = 'Date';
        $field['choices']['title'] = 'Title';
        $field['choices']['menu_order'] = 'Menu Order';
        $field['choices']['comment_count'] = 'Comment Count';
        $field['choices']['rand'] = 'Random';
        $field['choices']['ID'] = 'ID';
        
    return $field;
}
add_filter('acf/load_field/name=orderby', 'ppx_orderby_fields');


//PPX-DYNFLDS 1.3 POPULATE ORDER FIELD
function ppx_order_fields( $field ) {

        $field['choices'] = array();
        $field['choices']['ASC'] = 'Ascending';
        $field['choices']['DESC'] = 'Descending';
        
    return $field;
}
add_filter('acf/load_field/name=order', 'ppx_order_fields');
