...
class cardView extends viewBase
{
    ...
    <?php  // CARD LOAD MORE BUTTON ----------------------------------------
        ...
        // Load more settings
        $pagination = $this->settings['card']['pagination'] ?: false;
        $pagination_type = $this->settings['card']['pagination_type'] ?: 'numbers';
        $anchor_id = $this->settings['card']['pagination_anchors'] ? $this->id : false;
        ...
        if($max_num_pages > 1 && $pagination && $pagination_type == 'load_more'): ?>
        ...
        <?php 
        elseif($max_num_pages > 1 && $pagination && $pagination_type == 'numbers'): 
            pagination_nav($this->query, $anchor_id); 
         endif; ?>
    ...
    public function renderPosts($list_wrap = false){
        ...
        $set_link_text      = $this->settings['card']['link_text'] ?: '';
        ...
        while($this->query->have_posts())
        {
            ...
            // Get card
            ppx_get_card(get_the_title(), get_the_excerpt(), $set_support_link ? $permalink : '', $set_support_img ? $thumbnail_id : 0, $set_img_align, $block_attr, $set_link_text);
        }
    }
}
